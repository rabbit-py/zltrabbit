...
def get_datetime_in_timezone(epoch_second: Union[int, float], timezone_code: str) -> datetime:
...
def strtotime(date: str, fromtz: str = None, totz: str = None) -> int:
    ...
    return int(datetime.strptime(date, fmt).replace(tzinfo=fromtz).astimezone(totz).timestamp())


def datetime_now_in_timezone(timezone_code: str = TIMEZONE_SHANGHAI) -> datetime:
    return datetime.now(pytz.timezone(timezone_code))


def get_timezone(timezone_code: str = TIMEZONE_SHANGHAI) -> pytz.timezone:
...
def convert_timestamp_to_date(timestamp: Union[int, float]) -> str:
...
def get_hour_delta(from_timestamp=None, to_timestamp=None, from_date=None, to_date=None, str_format: str = '%Y-%m-%d %H:%M:%S'):
    """
